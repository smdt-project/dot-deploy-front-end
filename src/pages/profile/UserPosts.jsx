...
import PostModal from "../community/features/comSidebar/PostModal";
import { deleteItemRequest } from "./profileSlice";

const Post = ({ post, isLoggedInUser }) => {
	...
	const detailPost = () => navigateTo(`/community/post/${post._id}`);

	const handleDelete = () => {
		dispatch(deleteItemRequest({ itemName: "post", id: post._id }));
		setIsDeleting(false);
	};
	...
	return (
		<div className="sd:w-2/3 flex flex-col gap-3 bg-slate-900 bg-opacity-70 border-[1px] border-slate-800 rounded-md p-3">
			...
			{isEditing && (
				<PostModal isUpdating={true} post={post} setIsEditing={setIsEditing} />
			)}
			{showComment && (
...
const UserPosts = ({ isLoggedInUser }) => {
	...
	return posts.length > 0 ? (
		<div className="flex flex-col justify-center sd:items-start gap-3 sd:gap-7 px-2 sd:flex-row sd:flex-wrap pb-3">
			{posts.map((post, index) => (
				<Post post={post} key={index} isLoggedInUser={isLoggedInUser} />
			))}
		</div>
	) : (
